	...
	public partial class FormBrowser : Form, IBrowser
	{
		...
		public void ApplyZoom()
		{
			...
			try
			{
				...
				if (fit)
				{
					ToolMenu_Other_Zoom_Current.Text = string.Format("Current: Zoom To Fit");
				}
				else
				{
					ToolMenu_Other_Zoom_Current.Text = string.Format("Current: {0}%", zoomRate);
				}
			...
			{
				AddLog(3, "Failed to apply zoom. " + ex.Message);
			}
		...
		private Bitmap TakeScreenShot(bool is32bpp)
		{
			...
			try
			{
				...
				if (wb.Document.Url.ToString().Contains(".swf?"))
				{
					...
					if (viewobj == null)
					{
						throw new InvalidOperationException("Failed to load an embed tag.");
					}
				...
				else
				{
					...
					if (swf == null)
					{
						throw new InvalidOperationException("Could not find SWF file.");
					}
			...
			catch (Exception ex)
			{
				...
				SendErrorReport(ex.ToString(), "An error has occurred while capturing screenshot.");
				System.Media.SystemSounds.Beep.Play();
			}
