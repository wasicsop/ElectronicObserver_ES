...
public sealed class RedShipExportMap : ClassMap<ShipExportModel>
{
	...
	public RedShipExportMap(string prefix)
	{
		...
		Map(m => m.ConditionAfterBattle).Name(CsvExportResources.ConditionAfterBattle);
		...
		Map(m => m.FuelAfterBattle).Name(CsvExportResources.FuelAfterBattle);
		Map(m => m.FuelMax).Name(CsvExportResources.FuelMax);
		Map(m => m.AmmoCurrent).Name(CsvExportResources.AmmoCurrent);
		Map(m => m.AmmoAfterBattle).Name(CsvExportResources.AmmoAfterBattle);
		...
		References<RedEquipmentExportMap>(s => s.Equipment1).Prefix($"{prefix}{CsvExportResources.PrefixEquipment1}");
		References<RedEquipmentExportMap>(s => s.Equipment2).Prefix($"{prefix}{CsvExportResources.PrefixEquipment2}");
		References<RedEquipmentExportMap>(s => s.Equipment3).Prefix($"{prefix}{CsvExportResources.PrefixEquipment3}");
		References<RedEquipmentExportMap>(s => s.Equipment4).Prefix($"{prefix}{CsvExportResources.PrefixEquipment4}");
		References<RedEquipmentExportMap>(s => s.Equipment5).Prefix($"{prefix}{CsvExportResources.PrefixEquipment5}");
		References<RedEquipmentExportMap>(s => s.Equipment6).Prefix($"{prefix}{CsvExportResources.PrefixEquipment6}");
	}
}
