	...
	public partial class FormBattle : DockContent
	{
		...
		private void SetFormation(BattleManager bm)
		{
			...
			if (bm.IsEnemyCombined && bm.StartsFromDayBattle)
			{
				...
				FleetEnemy.BackColor = willMain ? Color.LightSteelBlue : Utility.Configuration.Config.UI.BackColor;
				FleetEnemyEscort.BackColor = willMain ? Utility.Configuration.Config.UI.BackColor : Color.LightSteelBlue;
			}
			else
			{
		...
		private void SetNightBattleEvent(PhaseNightInitial pd)
		{
			...
			//照明弾投射判定
			{
				...
				if (index != -1)
				{
					...
					ToolTipInfo.SetToolTip(AirStage2Friend, GeneralRes.StarShellUsed + ": " + pd.FlareFriendInstance.NameWithLevel);
