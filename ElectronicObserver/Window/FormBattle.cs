...
namespace ElectronicObserver.Window
{
	...
	public partial class FormBattle : DockContent
	{
		...
		private readonly Color WinRankColor_Win = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MainFontColor);
		...
		private void SetHPBar(BattleData bd)
		{
			...
			void EnableHPBar(int index, int initialHP, int resultHP, int maxHP)
			{
				...
				HPBars[index].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.BackgroundColor);
			...
			for (int i = 0; i < initial.FriendInitialHPs.Length; i++)
			{
				...
				if (initial.FriendInitialHPs[i] != -1)
				{
					...
					else bar.BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.BackgroundColor);
                }
				else
				{
			...
			if (!isBaseAirRaid)
			{
				foreach (int i in bd.MVPShipIndexes)
					HPBars[BattleIndex.Get(BattleSides.FriendMain, i)].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MVPHighlight);

                if (isFriendCombined)
				{
					foreach (int i in bd.MVPShipCombinedIndexes)
						HPBars[BattleIndex.Get(BattleSides.FriendEscort, i)].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MVPHighlight);
                }
			}
		...
		private void SetNightBattleEvent(PhaseNightInitial pd)
		{
			...
			//味方探照灯判定
			{
				...
				if (index != -1)
				{
					...
					AirStage1Friend.ForeColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MainFontColor);
			...
			//敵探照灯判定
			{
				...
				if (index != -1)
				{
					...
					AirStage1Enemy.ForeColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MainFontColor);
			...
			//照明弾投射判定
			{
				...
				if (index != -1)
				{
					...
					AirStage2Friend.ForeColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MainFontColor);
...

			{
				...
				if (index != -1)
				{
					...
					AirStage2Enemy.ForeColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MainFontColor);
		...
		private void SetMVPShip(BattleManager bm)
		{
			...
			for (int i = 0; i < friend.Members.Count; i++)
			{
				if (friend.EscapedShipList.Contains(friend.Members[i]))
					...
					HPBars[i].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MVPHighlight);

                else
					HPBars[i].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.BackgroundColor);
            }
			...
			if (escort != null)
			{
				for (int i = 0; i < escort.Members.Count; i++)
				{
					if (escort.EscapedShipList.Contains(escort.Members[i]))
						...
						HPBars[i + 6].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.MVPHighlight);

                    else
						HPBars[i + 6].BackColor = Utility.ThemeManager.GetColor(Utility.Theme.Dark, Utility.ThemeColors.BackgroundColor);
                }
			}
